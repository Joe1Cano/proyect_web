...
class ListsController extends Controller
{
    ...
    function createRoute($name){
        ...
        $routeContent = "\n\nuse $controller;\n\n";
        $routeContent .= "Route::resource('$name', $controller::class)->middleware('auth');\n";
        $routeContent .= "Route::get('/subir$name', [$controller::class,'subir$name'])->name('subir$name')->middleware('auth');\n";
        $routeContent .= "Route::delete('/delete$name/{id}', [$controller::class,'delete$name'])->name('delete$name')->middleware('auth');";    
    ...
    $content .= "    \$obj->l_list ='$name';\n";
    $content .= "    \$obj->save();\n";
    $content .= "    return redirect()->route('songs.index')->with('guardar', 'Canción agregada correcta');\n";
    $content .= "}\n";
    $content .= "public function delete$name(string \$id)\n";
    $content .= "{\n";
    $content .= "    \$obj = $name::destroy(\$id);\n";
    $content .= "    return redirect()->route('$name.index')->with('delete', 'Canción eliminada correctamente');\n";
    $content .= "}\n";
    $content .= "}";
