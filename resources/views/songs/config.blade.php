<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="{{ asset('css/navbar.css') }}">
    <link rel="stylesheet" href="{{ asset('css/config.css') }}">
    <script src="https://code.jquery.com/jquery-3.6.0.min.js"></script>
    <!--css link Bootstrap -->
    <link href="https://cdn.jsdelivr.net/npm/bootstrap@5.3.0-alpha1/dist/css/bootstrap.min.css" rel="stylesheet" integrity="sha384-GLhlTQ8iRABdZLl6O3oVMWSktQOp6b7In1Zl3/Jr59b6EGGoI1aFkw7cmDA6j6gD" crossorigin="anonymous">
    <!--css link Bootstrap icons -->
    <link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.10.3/font/bootstrap-icons.css">
    <link rel="icon" href="{{ asset('logos/LogoSinNombre.png') }}" type="image/x-icon">

    <title>Configuración</title>
</head>
<body>
    <div class="contenedor">

        <div id="navbar" class="sidenav" style="height: 100%;">
            <img class="img-fluid" id="logo" src="{{ asset('logos/LogoSinNombre.png') }}" alt="" id="logo" onclick="location.href='./songs'">
            <a href="./favorites"><span class="bi bi-heart-fill"></span> Favoritas</a>
            <a href="./listas"><span class="bi bi-music-note-list"></span> Playlists</a>
            <a href="./perfil"><span class="bi bi-person-lines-fill"> Perfil</a>
            <a href="./config"><span class="bi bi-sliders2-vertical"></span> Configuracion</a>
        </div>

        <div class="form">
            <div class="icon">
                <i class="bi bi-gear-fill" style="font-size: 100px"></i>
            </div>

            <form id="config" action="">
                <div class="cont" id="contuno">
                    <div class="text" id="uno">
                        <p>Añadir Playlist:</p>
                    </div>
                    <div class="button" id="uno">
                        <a href="./createList"><i class="bi bi-plus" style="font-size: 50px"></i></a>
                    </div>
                </div>
                <div class="cont" id="contdos">
                    <div class="text" id="dos">
                        <p>Añadir Canción:</p>
                    </div>
                    <div class="button" id="dos">
                        <a href="./subirSongs"><i class="bi bi-plus" style="font-size: 50px"></i></a>
                    </div>
                </div>
                <div class="cont" id="conttres">
                    <div class="text" id="tres">
                        <p>Modo oscuro:</p>
                    </div>
                    <div class="button" id="dos">
                        <a href="./subirSongs"><i class="bi bi-lightbulb" style="font-size: 50px"></i></a>
                    </div>
                </div>
                
                
            </form>
        </div>
    </div>


    
</body>
</html>