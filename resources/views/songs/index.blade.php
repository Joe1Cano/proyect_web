    ...
    @if(session('success'))
        <script>
            Swal.fire({
                ...
                //confirmButtonTextColor: '#ee62af',
                //confirmButtonColor: '#FFD3E7',
            })
        </script>
    @endif
    @if(session('saved'))
        ...
        <div class="tabla">
            ...
            <table style="width:100%">
                ...
                <tr id="trtbl" >
                    ...
                    <!--<td onclick=<?php echo "showData('".$song->id."')"?> id='covertbl'><input type="hidden" name="img" id="img" value="{{$song->foto}}"><img id="imgtbl" src=" <?php echo "http://localhost:80/proyecto_web/storage/app/public/".$song->foto ?>" ></td></td> -->
                    <td id='covertbl'><input type="hidden" name="img" id="img" value="{{$song->foto}}"><img id="imgtbl" src=" <?php echo "http://localhost:80/paw233/proyect_web/storage/app/public/".$song->foto ?>" >
                    <td onclick=<?php echo "showData('".$song->id."')"?>><input type="hidden" name="name" id="name" value="{{$song->nombre}}"><label id="nametbl">{{$song->nombre}}</label></td>
                    <td onclick=<?php echo "showData('".$song->id."')"?>><input type="hidden" name="autor" id="autor" value="{{$song->autor}}">{{$song->autor}}</td>
                    <td><button type="submit" id="liketbl"><span id="liketbl" class="bi bi-heart-fill"></button></td>
                    </form>

                    <form class="my-form" id="form{{$song->id}}" action="" method="get">
                        ...
                        <label for="mySelect">
                            <i class="bi bi-list-ul" id="list" onclick="abrirSelect()"></i>
                                <select class="mySelect" name="mySelect" id="{{$song->id}}" onclick="console.log({{$song->id}})">
...
/*function abrirSelect() {
    ...
    const select = document.getElementById("{{$song->id}}");
